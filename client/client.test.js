...
describe("client", () => {
  ...
  describe("should return results for", () => {
    ...
    it("fifteenth page with dominant colors", (done) => {
      var expected = {
        previousPage: 14,
        nextPage: 16,
        ids: [236, 237, 238, 240, 241, 243, 246, 251, 252, 256],
        for_sale: [
          {
            id: 240,
            dominant_colors: "blue",
      ...
      retrieve({ page: 15, dominant_colors: ["red", "blue", "brown"] })
        .then((output) => {
          expect(output).toEqual(expected);
        })
        .then(done);
    ...
    it("thirty fourth page", (done) => {
      var expected = {
        previousPage: 33,
        nextPage: 35,
        ids: [331, 332, 333, 334, 335, 336, 337, 338, 339, 340],
        for_sale: [
          {
            id: 332,
            dominant_colors: "brown",
  ...
  it("should return results filtered by multiple dominant colors", (done) => {
    var expected = {
      previousPage: null,
      nextPage: 2,
      ids: [5, 6, 10, 11, 15, 16, 17, 22, 23, 24],
      for_sale: [
        {
          id: 6,
          dominant_colors: "blue",
    ...
    retrieve({ page: 1, dominant_colors: ["red", "blue"] })
      .then((output) => {
        expect(output).toEqual(expected);
      })
      .then(done);
  ...
  it("should return results filtered by a single dominant color", (done) => {
    ...
    retrieve({ dominant_colors: ["brown"] })
      .then((output) => {
        expect(output).toEqual(expected);
      })
      .then(done);
  ...
  it("should return empty results", (done) => {
    ...
    retrieve({ dominant_colors: ["hotpink"] })
      .then((output) => {
        expect(output).toEqual(expected);
      })
      .then(done);
